import 'package:xml/xml.dart';

void main() {
  final xmlString = '''
<entry>
<updated>2025-12-10T17:37:17Z</updated>
<id>https://www.gutenberg.org/ebooks/289.opds</id>
<title>The Wind in the Willows</title>
<content type="text">Kenneth Grahame</content>
<link type="application/atom+xml;profile=opds-catalog" rel="subsection" href="/ebooks/289.opds"/>
<link type="image/png" rel="http://opds-spec.org/image/thumbnail" href="data:image/png;base64,"/>
</entry>
''';

  final document = XmlDocument.parse(xmlString);
  final entry = document.findAllElements('entry').first;

  String? thumbnail;

  for (final link in entry.findElements('link')) {
    final rel = link.getAttribute('rel');
    final href = link.getAttribute('href');

    print(
      'Link: rel=$rel href=${href != null && href.length > 20 ? href.substring(0, 20) : href}...',
    );

    if (rel != null && rel.contains('thumbnail')) {
      print('Found thumbnail match!');
      if (href != null && href.startsWith('data:')) {
        thumbnail = href;
        print('Set thumbnail from data URI');
      }
    }
  }

  print('Final thumbnail: ${thumbnail != null ? "FOUND" : "NULL"}');
}
